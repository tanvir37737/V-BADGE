                ...
                while True:
                    try:
                        ...
                        if self.DaTa.hex().startswith("1200") and b"SecretCode" in self.DaTa:
                            ...
                            msg_part1 = (
                                "-HELLO I AM TANVIR AHAMED  !\n\n"
                                "SUBSCRIBE ME ON YOUTUBE  OR BE BANNED \n\n"
                                "SBPRIME HERE : "
                            )
                            msg_part2 = "@psychotaanvir07 !! \n\n"
                            msg_part3 = (
                                "telegram team channel : @psychotaanvir07 \n\n"
                                "DEV Telegram username : @TANVIR_bro"
                            )
...
def StarT_SerVer():
    print(render("TANVIR", colors=["white", "yellow"], align="center"))
    TexT = f"[TarGeT InFo] > BoTs arE OnLine\n[BoT sTaTus] > [bold green]ConEcTed SuccEssFuLy[/bold green]"
    panel = Panel(
        Align.center(TexT),
        title="[bold yellow]FF - TANVIR[/bold yellow]",
...
# Added by Hustler web @aadix56
    ...
    forced_target_input = input("Enter target UID to force (IF YOU ARE BAD _-_-_TANVIR IS YOUR DAD🥶): ").strip()
